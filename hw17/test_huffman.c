...
int main(int argc, char* argv[]) {
    ...
    // // -----Testing for HW17-------------------------//
    FILE* open_file = fopen("expected.txt","r");
    uint8_t output;
    
    for(unsigned char ch = fgetc(open_file); !feof(open_file); ch = fgetc(open_file))
        {
            output = ch;
        }
    BitWriter writer = open_bit_writer("huffman_output_code");
    write_compressed(tree_head, &writer, &output);
    close_bit_writer(&writer);
    destroy_huffman_tree(&tree_head); 
